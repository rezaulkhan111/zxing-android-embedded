...
import com.journeyapps.barcodescanner.camera.CameraConfigurationUtils.setBarcodeSceneMode
import com.journeyapps.barcodescanner.camera.CameraConfigurationUtils.setBestExposure
import com.journeyapps.barcodescanner.camera.CameraConfigurationUtils.setBestPreviewFPS
import com.journeyapps.barcodescanner.camera.CameraConfigurationUtils.setFocus
import com.journeyapps.barcodescanner.camera.CameraConfigurationUtils.setFocusArea
import com.journeyapps.barcodescanner.camera.CameraConfigurationUtils.setInvertColor
import com.journeyapps.barcodescanner.camera.CameraConfigurationUtils.setMetering
import com.journeyapps.barcodescanner.camera.CameraConfigurationUtils.setTorch
import com.journeyapps.barcodescanner.camera.CameraConfigurationUtils.setVideoStabilization
...
class CameraManager(private val context: Context) {
     ...
     */
    var cameraRotation: Int = -1 // camera rotation vs display rotation
        private set
    ...
    private inner class CameraPreviewCallback : Camera.PreviewCallback {
        ...
        override fun onPreviewFrame(data: ByteArray, camera: Camera) {
            ...
            if (cameraResolution != null && callback != null) {
                try {
                    if (data == null) {
                        throw NullPointerException("No preview data received")
                    }
                    val format = camera.parameters.previewFormat
                    val source = SourceData(
                        data, cameraResolution.width, cameraResolution.height, format,
                        this.cameraRotation
                    )
    ...
    private fun setDesiredParameters(safeMode: Boolean) {
        ...
        setFocus(parameters, cameraSettings.focusMode, safeMode)

        if (!safeMode) {
            setTorch(parameters, false)
        ...
        if (previewSizes.size == 0) {
            ...
            requestedPreviewSize = displayConfiguration!!.getBestPreviewSize(
                previewSizes,
                isCameraRotated
            )
    ...
    private fun calculateDisplayRotation(): Int {
        ...
        Log.i(
            TAG,
            "Camera Display Orientation: $result"
        )
        return result
    }
    ...
    fun getPreviewSize(): Size? {
        return if (naturalPreviewSize == null) {
            null
    ...
    companion object {
        private val TAG: String = CameraManager::class.java.simpleName

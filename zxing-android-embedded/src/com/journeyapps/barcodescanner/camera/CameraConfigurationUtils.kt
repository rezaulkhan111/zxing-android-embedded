...
object CameraConfigurationUtils {
    ...
    fun setFocus(
        ...
        if (focusMode != null) {
            if (focusMode == parameters.focusMode) {
                Log.i(
                    TAG,
                    "Focus mode already set to $focusMode"
                )
            } else {
    ...
    fun setTorch(parameters: Camera.Parameters, on: Boolean) {
        ...
        if (flashMode != null) {
            if (flashMode == parameters.flashMode) {
                Log.i(
                    TAG,
                    "Flash mode already set to $flashMode"
                )
            } else {
    ...
    fun setBestExposure(parameters: Camera.Parameters, lightOn: Boolean) {
        ...
        if ((minExposure != 0 || maxExposure != 0) && step > 0.0f) {
            ...
            // Clamp value:
            compensationSteps = max(
                min(compensationSteps.toDouble(), maxExposure.toDouble()),
    ...
    fun setBestPreviewFPS(parameters: Camera.Parameters, minFPS: Int, maxFPS: Int) {
        ...
        if (supportedPreviewFpsRanges != null && !supportedPreviewFpsRanges.isEmpty()) {
            ...
            if (suitableFPSRange == null) {
                ...
                if (currentFpsRange.contentEquals(suitableFPSRange)) {
                    Log.i(TAG, "FPS range already set to " + suitableFPSRange.contentToString())
                } else {
                    Log.i(TAG, "Setting FPS range to " + suitableFPSRange.contentToString())
    ...
    fun setZoom(parameters: Camera.Parameters, targetZoomRatio: Double) {
        if (parameters.isZoomSupported) {
            val zoom =
                indexOfClosestZoom(parameters, targetZoomRatio) ?: return
    ...
    private fun findSettableValue(
        ...
        Log.i(TAG, "Requesting " + name + " value from among: " + desiredValues.contentToString())

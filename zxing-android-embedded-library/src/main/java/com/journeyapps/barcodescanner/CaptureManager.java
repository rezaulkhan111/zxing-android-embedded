...
import com.journeyapps.barcodescanner.google.BeepManager;
import com.journeyapps.barcodescanner.google.InactivityTimer;
...
public class CaptureManager {
    ...
    private final CameraPreview.StateListener stateListener = new CameraPreview.StateListener() {
        ...
        public void cameraError(Exception error) {
            displayFrameworkBugMessageAndExit(activity.getString(R.string.zxing_msg_camera_framework_bug));
        }
    ...
    private void openCameraWithPermission() {
        if (ContextCompat.checkSelfPermission(this.activity, Manifest.permission.CAMERA) == PackageManager.PERMISSION_GRANTED) {
            ...
            ActivityCompat.requestPermissions(this.activity, new String[]{Manifest.permission.CAMERA}, cameraPermissionReqCode);
    ...
    public static Intent resultIntent(BarcodeResult rawResult, String barcodeImagePath) {
        ...
        if (rawResult.getText() == Util.dataNotFound_Key) {
            intent.putExtra(Intents.Scan.RESULT, Util.dataNotFound_Key);
        } else {
